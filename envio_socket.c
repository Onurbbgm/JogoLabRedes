...
int main()
{
  ...
  /* Configura MAC Origem e Destino */
   MacAddress localMac = {0x00, 0x0D, 0xF0, 0x87, 0x12, 0xD3};
   MacAddress destMac = {0x00, 0x0D, 0xF0, 0x87, 0x12, 0xD3};
//   MacAddress destlMac = {0xAC, 0xBC, 0x32, 0x79, 0xA1, 0xA1};
  ...
  //  if((retValue = sendto(sockFd, buffer, 64, 0, (struct sockaddr *)&(destAddr), sizeof(struct sockaddr_ll))) < 0) {
    //   printf("ERROR! sendto() \n");
      // exit(1);
    //}
    if((retValue = sendto(sockFd, server_message, sizeof(server_message), 0,(struct sockaddr *)&(destAddr), sizeof(struct sockaddr_ll))) < 0) {
